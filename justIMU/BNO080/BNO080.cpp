...
BNO080Base::BNO080Base(BufferedSerial* debugPort, PinName user_INTPin, PinName user_RSTPin) :
...
bool BNO080Base::begin()
{
	...
	while(true)
	{
		...
		{
			printf("Error: BNO080 reset timed out, chip not detected.\n");
			return false;
		}
	...
	{
		printf("No initialization report from BNO080.\n");
		return false;
	}
	else
	{
#if BNO_DEBUG
		printf("BNO080 reports initialization successful!\n");
#endif
	}
...
#if BNO_DEBUG
		printf("BNO080 reports as SW version %hhu.%hhu.%hu, build %lu, part no. %lu\n",
	...
	{
		printf("Bad response from product ID command.\n");
		return false;
	}
...
bool BNO080Base::enableCalibration(bool calibrateAccel, bool calibrateGyro, bool calibrateMag)
{
	...
	{
#if BNO_DEBUG
		printf("Timeout waiting for calibration response!\n");
#endif
		return false;
	}
	...
	{
#if BNO_DEBUG
		printf("Received wrong response to calibration command!\n");
#endif
		return false;
	}
	...
	{
#if BNO_DEBUG
		printf("IMU reports calibrate command failed!\n");
#endif
		return false;
	}
...
void BNO080Base::printMetadataSummary(Report report)
{
	...
	printf("======= Metadata for report 0x%02hhx =======\n", static_cast<uint8_t>(report));

	printf("Range: +- %.04f units\n", getRange(report));
	printf("Resolution: %.04f units\n", getResolution(report));
	printf("Power Used: %.03f mA\n", getPower(report));
	printf("Min Period: %.06f s\n", getMinPeriod(report));
	printf("Max Period: %.06f s\n\n", getMaxPeriod(report));
...
void BNO080Base::parseSensorDataPacket()
{
	...
	while(currReportOffset < rxPacketLength)
	{
		...
		switch(rxShtpData[currReportOffset])
		{
			...
			default:
				printf("Error: unrecognized report ID in sensor report: %hhx.  Byte %u, length %hu\n", rxShtpData[currReportOffset], currReportOffset, rxPacketLength);
				return;
		}

		if(currReportOffset >= SHTP_RX_PACKET_SIZE)
		{
			printf("Error: sensor report longer than packet buffer! Some data was not read! Increase buffer size or decrease number of reports!\r\n");
			return;
		}
	}
...
bool BNO080Base::readFRSRecord(uint16_t recordID, uint32_t* readBuffer, uint16_t readLength)
{
	...
	while(readOffset < readLength)
	{
		...
		{
#if BNO_DEBUG
			printf("Error: FRS reports invalid record ID!\n");
#endif
			return false;
		}
		else if(status == 2)
		...
		{
#if BNO_DEBUG
			printf("Error: FRS reports offset is out of range!\n");
#endif
			return false;
		}
		else if(status == 5)
		{
#if BNO_DEBUG
			printf("Error: FRS reports record %hx is empty!\n", recordID);
#endif
			return false;
		}
		else if(status == 8)
		{
#if BNO_DEBUG
			printf("Error: FRS reports flash memory device unavailable!\n");
#endif
			return false;
		}
		...
		{
#if BNO_DEBUG
			printf("Error: Received FRS packet with 0 data length!\n");
#endif
			return false;
		}
		else if(dataLength == 1)
		{
			...
			{
#if BNO_DEBUG
				printf("Error: Received 1 length packet but more than 1 byte remains to be be read!\n");
#endif
				return false;
			}
		}
...
bool BNO080Base::writeFRSRecord(uint16_t recordID, uint32_t* buffer, uint16_t length)
{
	...
	{
#if BNO_DEBUG
		printf("Error: did not receive FRS write ready response after sending write request!\r\n");
#endif
		return false;
	}
	...
	{
#if BNO_DEBUG
		printf("Error: FRS reports error initiating write operation: %hhu!\r\n", rxShtpData[1]);
#endif
		return false;
	}
	...
	for(uint16_t wordIndex = 0; wordIndex < length; wordIndex += 2)
	{
		...
		*reinterpret_cast<uint32_t*>(txShtpData + 4) = buffer[wordIndex];
		...
		{
#if BNO_DEBUG
			printf("Error: did not receive FRS write response after sending write data!\r\n");
#endif
			return false;
		}
		...
		switch(status)
		{
			case 0:
				...
				{
#if BNO_DEBUG
					printf("Error: FRS reports write in progress when it should be complete!\r\n");
#endif
					return false;
				}
				break;
			...
			case 8:
				...
				{
#if BNO_DEBUG
					printf("Error: FRS reports write complete when it should be still going!\n");
#endif
					return false;
				}
				break;
			...
			case 5:
#if BNO_DEBUG
				printf("Error: FRS reports write failed!\n");
#endif
				return false;
			case 6:
#if BNO_DEBUG
				printf("Error: FRS reports data received while not in write mode!\n");
#endif
				return false;
			case 7:
#if BNO_DEBUG
				printf("Error: FRS reports invalid length!\n");
#endif
				return false;
			case 9:
#if BNO_DEBUG
				printf("Error: FRS reports invalid data for this record!\n");
#endif
				return false;

			case 10:
#if BNO_DEBUG
				printf("Error: FRS reports flash device unavailable!\n");
#endif
				return false;

			case 11:
#if BNO_DEBUG
				printf("Error: FRS reports record is read-only!\n");
#endif
				return false;
			default:
#if BNO_DEBUG
				printf("Error: FRS reports unknown result code %hhu!\n", status);
#endif
				break;
...
void BNO080Base::printPacket(uint8_t * buffer)
{
	...
	{
		printf(" ");
		printf("%02hhx", buffer[x + SHTP_HEADER_SIZE]);
	}

	printf(", Length:");
	printf("%d", packetLength + SHTP_HEADER_SIZE);
	printf(", SeqNum: %hhu", buffer[3]);

	printf(", Channel:");
	if (buffer[2] == 0) printf("Command");
	else if (buffer[2] == 1) printf("Executable");
	else if (buffer[2] == 2) printf("Control");
	else if (buffer[2] == 3) printf("Sensor-report");
	else if (buffer[2] == 4) printf("Wake-report");
	else if (buffer[2] == 5) printf("Gyro-vector");
	else printf("%hhu", buffer[2]);
...
BNO080I2C::BNO080I2C(BufferedSerial *debugPort, PinName user_SDApin, PinName user_SCLpin, PinName user_INTPin, PinName user_RSTPin,
...
bool BNO080I2C::receivePacket(std::chrono::milliseconds timeout)
{
	...
	while(_int.read() != 0)
	{
		...
		{
			printf("BNO I2C header wait timeout\n");
			return false;
		}
	...
	if(i2cResult != 0)
	{
		printf("BNO I2C length read failed!\n");
		return false;
	}
	...
	printf("Attempting to read %" PRIu16 " bytes\n", totalLength);
...
bool BNO080SPI::sendPacket(uint8_t channelNumber, uint8_t dataLength)
{
	...
	else
	{
		...
		while(_int.read() != 0)
		{
			if(waitStartTime.elapsed_time() > timeout)
			{
				printf("BNO SPI wake wait timeout\n");
				_wakePin = 1;
				return false;
			}
		}
...
bool BNO080SPI::receivePacket(std::chrono::milliseconds timeout)
{
	...
	while (_int.read() != 0)
	{
		if (waitStartTime.elapsed_time() > timeout)
		{
			printf("BNO SPI header wait timeout\n");
			return false;
		}
...
bool BNO080SPI::receiveCompletePacket(size_t bytesRead, std::chrono::milliseconds timeout)
{
	...
	while (_int.read() != 0)
	{
		if (waitStartTime.elapsed_time() > timeout)
		{
			printf("BNO SPI continued packet header wait timeout\n");
			return false;
		}
	...
	{
		printf("Packet too long (%" PRIu16 " bytes), increase SHTP_RX_PACKET_SIZE\n", rxPacketLength);
		printf("Packet dropped, expect subsequent driver errors.\n");
		return false;
	}
...
void BNO080SPI::spiTransferAndWait(const uint8_t *tx_buffer, int tx_length, uint8_t *rx_buffer, int rx_length)
{
	...
	if(!(waitResult & SPI_EVENT_COMPLETE))
	{
		...
		printf("BNO Async SPI Error %" PRIu32 "\n", waitResult);
	}
